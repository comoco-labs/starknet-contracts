...
@event
func AuthorChanged(tokenId: Uint256, previousAuthor: felt, newAuthor: felt) {
}
...
namespace Authorable {
    ...
    func set_author{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
        ...
        with_attr error_message("Authorable: author is the zero address") {
            assert_not_zero(new_author);
        }
        let (previous_author) = Authorable_authors.read(token_id);
        Authorable_authors.write(token_id, new_author);
        AuthorChanged.emit(token_id, previous_author, new_author);
        return ();
    }
